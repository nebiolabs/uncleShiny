...
# SERVER
scatter <- function(input, output, session, data, loadBtn, expSel) {
  
  # Loading in the plot options module as reactive values..
  plotOpts <- callModule(plotOpts, "plotOpts")
  ...
  observe({
    ...
    callModule(specSpark, "specSparkHover", hoverData_summary(), clickData_summary(), plotOpts$palette())
  })
  ...
  callModule(spectraViewer, "spectraViewer", spectraData)
